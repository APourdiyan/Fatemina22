...
export default function ContentDetail() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/archive" className="text-primary hover:underline">بازگشت به آرشیو</Link>
        </div>
      </div>
    );
  }
  ...
  const relatedLectures = sampleLectures
    .filter((l) => l.id !== lecture.id && (l.topic === lecture.topic || l.tags?.some((t) => lecture.tags?.includes(t))))
    .slice(0, 4);
  ...
  const handleCopyLink = () => {
    ...
    toast({ title: "لینک کپی شد", description: "لینک این محتوا در کلیپ‌بورد کپی شد." });
  };
        ...
        <div className="bg-secondary/50 border-b border-border/50">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-3">
            <nav className="flex items-center gap-2 text-sm text-muted-foreground" aria-label="breadcrumb">
              ...
              <Link to="/archive" className="hover:text-foreground transition-colors">{lecture.topic}</Link>
              <ChevronLeft className="w-3.5 h-3.5" />
              <Link to="/archive" className="hover:text-foreground transition-colors">{lecture.series}</Link>
              <ChevronLeft className="w-3.5 h-3.5" />
              <span className="text-foreground font-medium truncate max-w-[200px]">{lecture.title}</span>
            </nav>
          </div>
        </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-6">
                  ...
                  <span className="text-xs bg-secondary px-2 py-0.5 rounded-full">{lecture.series}</span>
                </div>
              </motion.header>
              ...
              {lecture.transcript && lecture.transcript.length > 0 && (
                <TranscriptPanel transcript={lecture.transcript} lectureId={lecture.id} />
              )}

              {/* Related Content */}
              {relatedLectures.length > 0 && (
                <RelatedContent lectures={relatedLectures} />
              )}
            </div>
